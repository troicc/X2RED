...
def upgrade() -> None:
    with op.batch_alter_table("source_items") as batch:
        batch.add_column(sa.Column("workspace_state", sa.String(length=20), nullable=False, server_default="active"))
        batch.add_column(sa.Column("content_kind", sa.String(length=30), nullable=False, server_default="post"))
        batch.add_column(sa.Column("structured_content_json", sa.Text(), nullable=False, server_default="{}"))
        batch.add_column(sa.Column("editor_note", sa.Text(), nullable=False, server_default=""))
        batch.add_column(sa.Column("archived_at", sa.DateTime(timezone=True), nullable=True))
        batch.add_column(sa.Column("last_published_at", sa.DateTime(timezone=True), nullable=True))
        batch.add_column(sa.Column("published_count", sa.Integer(), nullable=False, server_default="0"))
        batch.create_index("ix_source_items_workspace_state", ["workspace_state"], unique=False)
    ...
    op.create_index("ix_skill_bindings_skill_name", "skill_bindings", ["skill_name"], unique=True)
