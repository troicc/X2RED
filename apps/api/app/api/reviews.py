...
def decide_artifact(
    ...
    if artifact is None:
        raise HTTPException(status_code=404, detail="审阅产物不存在")
    try:
        artifact = _service(request).decide(
            db,
            artifact,
...
def publisher_payload(
    ...
    try:
        return _service(request).publisher_payload(db, variant)
    except ReviewFlowError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
def publisher_extension() -> StreamingResponse:
    extension_dir = Path(__file__).resolve().parents[2] / "extensions" / "wechat-publisher-assistant"
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": 'attachment; filename="x2red-wechat-publisher-assistant.zip"'
        },
    )
