...
from app.domain.models import DraftRevision, SkillBinding, SourceItem
...
_KEYWORD_TAGS = {
    "openai": "OpenAI", "chatgpt": "ChatGPT", "agent": "AI智能体", "model": "大模型",
    "llm": "大模型", "artificial intelligence": "人工智能", " ai ": "人工智能",
    "workflow": "效率工具", "local-first": "本地优先", "xiaohongshu": "小红书",
    "小红书": "小红书", "python": "Python", "product": "产品观察",
    "startup": "创业观察", "design": "设计思考", "creator": "内容创作",
    "content": "内容创作", "interface": "界面设计", "navigation": "交互设计",
    "ux": "UX设计", "ui": "UI设计",
}
_ANALYSIS_FIELDS = (
    "topic", "one_sentence_summary", "verified_facts", "author_claims", "uncertainties",
...
class EditorialService:
    ...
    async def generate(self, db: Session, source: SourceItem, style: str) -> DraftRevision:
        ...
        model_result = await self._model_generate(context, style, bindings)
        analysis: dict = {}
    ...
    def revise(self, db: Session, current: DraftRevision, *, title: str, body: str, tags: str) -> DraftRevision:
        revised = DraftRevision(
            source_id=current.source_id,
            version=self._next_version(db, current.source_id, current.version),
            style=current.style,
            title=title.strip(), body=body.strip(), tags=tags.strip(),
            claims_json=current.claims_json, provenance_json=current.provenance_json,
            created_by="human",
        )
        db.add(revised)
        db.flush()
...
""".strip()
        ...
        body = current.body if action == "rewrite_title" else str(parsed.get("body") or current.body)
        tags = current.tags if action == "rewrite_title" else self._tags_value(parsed.get("tags")) or current.tags
        generated = self._sanitize_generated(
            {"title": title, "body": body, "tags": tags, "claims": claims}, context, current.style
        )
        passes = list(provenance.get("quality_passes") or []) if isinstance(provenance, dict) else []
        passes.append(action)
        next_provenance = {
            **(provenance if isinstance(provenance, dict) else {}),
            "generator": "model-transform", "model": binding.model_name or self.settings.model_name,
            "parent_draft_id": current.id, "transform_action": action,
        ...
        revised = DraftRevision(
            ...
            version=self._next_version(db, current.source_id, current.version),
            style=current.style,
            title=generated["title"][:80], body=generated["body"][:4000], tags=generated["tags"][:500],
            claims_json=json.dumps(generated["claims"], ensure_ascii=False),
            provenance_json=json.dumps(next_provenance, ensure_ascii=False), created_by="model-polish",
        )
        db.add(revised)
        db.flush()
        return revised
    ...
    def _next_version(db: Session, source_id: str, fallback: int = 0) -> int:
        current = db.scalar(select(func.max(DraftRevision.version)).where(DraftRevision.source_id == source_id))
        return int(current or fallback) + 1

    @staticmethod
    def _binding_defaults(name: str, settings: Settings) -> SkillBinding:
        effort = "high" if name == "editorial.analysis" else "low" if name == "writing.de_translate" else "medium"
        return SkillBinding(skill_name=name, enabled=True, model_name=settings.model_name, reasoning_effort=effort)
...
""".strip()
        try:
            analysis = await self._chat_json(
                ...
                user_prompt=analysis_prompt, temperature=0.2,
...
""".strip()
            initial = await self._chat_json(
                ...
                user_prompt=writing_prompt, temperature=0.5,
            ...
            if polish_binding.enabled:
                final = await self._polish_draft(initial, analysis, source_json, polish_binding)
            ...
            return {
                ...
                "draft": {
                    ...
                    "tags": self._tags_value(final.get("tags")) or self._tags_value(initial.get("tags")),
    ...
    async def _polish_draft(
        self, initial: dict, analysis: dict, source_json: str, binding: SkillBinding
    ) -> dict:
        prompt = f"""
...
""".strip()
        try:
            return await self._chat_json(
                ...
                user_prompt=prompt, temperature=0.35,
                reasoning_effort=binding.reasoning_effort, model_name=binding.model_name,
            )
        except (httpx.HTTPError, KeyError, TypeError, ValueError, json.JSONDecodeError):
            return initial
    ...
    async def _chat_json(
        ...
        reasoning_effort: str, model_name: str = "",
    ) -> dict:
        ...
        without_format = dict(request_body); without_format.pop("response_format", None); variants.append(without_format)
        if "thinking" in request_body or "reasoning_effort" in request_body:
            portable = dict(without_format); portable.pop("thinking", None); portable.pop("reasoning_effort", None); variants.append(portable)
    ...
    def _source_blocks(context: list[SourceItem]) -> list[dict]:
        ...
        for index, item in enumerate(context, start=1):
            try: metrics = json.loads(item.metrics_json or "{}")
            except json.JSONDecodeError: metrics = {}
            created = item.created_at.isoformat() if isinstance(item.created_at, datetime) else ""
            output.append({
                "index": index, "author": item.author_name, "handle": item.author_handle,
                "published_at": created, "metrics": metrics, "content_kind": item.content_kind,
                "text": item.text_original, "editor_note": item.editor_note, "url": item.canonical_url,
            })
        return output
    ...
    def _parse_json_object(content: str) -> dict:
        ...
        if not isinstance(parsed, dict): raise ValueError("model response is not a JSON object")
        return parsed
    ...
    def _map_model_claims(value: object, context: list[SourceItem]) -> list[dict]:
        ...
        for raw in value:
            ...
            try: source_index = max(1, int(raw.get("source_index") or 1))
            ...
            if verification not in {"source_only", "needs_external_check"}: verification = "source_only"
            statement = str(raw.get("statement") or "").strip()
            if statement:
                claims.append({"statement": statement[:300], "source_id": item.id, "source_url": item.canonical_url, "verification": verification})
        return claims

    def _fallback(self, context: Iterable[SourceItem], style: str) -> dict:
        items = list(context); focal = items[0]
        cleaned_pairs = [(item, self._clean(item.text_original)) for item in items if item.text_original.strip()]
        pieces: list[tuple[SourceItem, str]] = []
        for item, text in cleaned_pairs:
            for sentence in self._sentences(text):
                if sentence and all(sentence != existing for _, existing in pieces): pieces.append((item, sentence))
        ...
        title = self._fallback_title(first, handle, has_chinese); summary = self._trim_sentence(first, 110)
        supporting = [self._trim_sentence(text, 120) for _, text in pieces[1:5]]
        if style == "news":
            parts = ["发生了什么", f"@{handle} 发布的信息可概括为：{summary}", "核心信息", *[f"• {x}" for x in supporting[:3]], "仍需确认", "涉及数字、时间、效果或因果判断时，应结合公开资料核查。"]
        elif style == "opinion":
            parts = ["我的判断", f"这更像一个观察信号，而不是完整结论。{summary}", "判断依据", *[f"• {x}" for x in ([summary, *supporting][:3])], "给读者的判断框架", "先确认它改变了什么具体行为，再判断是不是可复制趋势。"]
        else:
            parts = ["先说结论", f"@{handle} 的核心信息是：{summary}", "值得关注的 3 个点"]
            for index, point in enumerate([summary, *supporting][:3], start=1): parts.append(f"{index}️⃣ {point}")
            parts.extend(["这对读者有什么用", "它提供了一条一手线索，适用条件与实际效果仍需验证。", "阅读提醒", "本文基于已归档 X 来源整理。"])
        claims = [{"statement": text[:300], "source_id": item.id, "source_url": item.canonical_url, "verification": "source_only"} for item, text in pieces[:8]]
        return {"title": title, "body": "\n\n".join(parts), "tags": ",".join(self._fallback_tags(focal, style, " ".join(text for _, text in pieces))), "claims": claims}

    def _sanitize_generated(self, generated: dict, context: list[SourceItem], style: str) -> dict:
        title = re.sub(r"\s+", " ", re.sub(r"^[#\s]+|[#\s]+$", "", str(generated.get("title") or ""))).strip("，。！？；：-—")
        body = re.sub(r"\n{3,}", "\n\n", str(generated.get("body") or "").replace("\r\n", "\n").strip())
        if not title:
            focal = context[0]; title = self._fallback_title(self._clean(focal.text_original), focal.author_handle or focal.author_name or "原作者", bool(re.search(r"[\u4e00-\u9fff]", focal.text_original)))
        if not body: body = self._fallback(context, style)["body"]
        tags = []
        for value in re.split(r"[,，#\n]+", self._tags_value(generated.get("tags"))):
            tag = re.sub(r"\s+", "", value).strip()
            if 1 < len(tag) <= 18 and tag not in tags: tags.append(tag)
        if len(tags) < 4: tags.extend(tag for tag in self._fallback_tags(context[0], style, body) if tag not in tags)
    ...
    def _fallback_title(text: str, handle: str, has_chinese: bool) -> str:
        cleaned = re.sub(r"\s+", " ", re.sub(r"[@#]\S+", "", re.sub(r"https?://\S+", "", text))).strip("，。！？；：-— ")
        if has_chinese and cleaned:
            first = re.split(r"[。！？\n]", cleaned)[0].strip()
            if first: return first[:22].rstrip("，。！？；：")
    ...
    def _fallback_tags(self, focal: SourceItem, style: str, text: str) -> list[str]:
        normalized = f" {text.lower()} "; tags = [{"news": "科技资讯", "opinion": "编辑观察"}.get(style, "信息拆解"), "X平台观察"]
        for needle, tag in _KEYWORD_TAGS.items():
            if needle in normalized and tag not in tags: tags.append(tag)
        handle_tag = re.sub(r"[^A-Za-z0-9_\u4e00-\u9fff]", "", focal.author_handle or "")
        if handle_tag and handle_tag not in tags: tags.append(handle_tag[:18])
        if "内容创作" not in tags: tags.append("内容创作")
        return tags[:7]

    @staticmethod
    def _sentences(text: str) -> list[str]:
        normalized = re.sub(r"\s+", " ", text).strip()
        return [part.strip() for part in re.split(r"(?<=[。！？!?；;])\s*|(?<=\.)\s+(?=[A-Z0-9])", normalized) if len(part.strip()) >= 4] if normalized else []
