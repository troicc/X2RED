...
class HtmlCardRenderer:
    ...
    def _document(self, spec: dict, template: str) -> str:
        ...
        kind = html.escape(str(spec.get("kind") or "key_takeaways"))
        ...
        is_cover = kind == "hero_cover"
        title_size = "92" if is_cover else "74"
        title_margin = "160" if is_cover else "125"
...
.kicker{{font-size:25px;font-weight:800;letter-spacing:.08em;color:{accent}}}.counter{{font-size:24px;font-weight:750;color:{fg}88}}
...
.hero{{position:absolute;left:72px;right:72px;bottom:180px;height:570px;border-radius:34px;background-size:cover;background-position:center;box-shadow:0 18px 48px #0003}}
...
</style></head><body><main class="card"><article class="frame {kind}"><div class="top"><div class="kicker">{kicker}</div><div class="counter">{page:02d} / {total:02d}</div></div><h1>{title}</h1><div class="rule"></div>{hero_html}{content_html}</article></main></body></html>"""
