...
2. 深度研究与审稿：可选，输出任务单、证据包、大纲、完整初稿、三路审稿、修改裁决和深度终稿；
...
每个阶段必须显示当前状态、读取、输出或修改、Skill/实际模型路由和建议优化点，不能只展示一个模糊的“生成中”。深度写作详情进一步展开为八步：任务定义、证据研究、文章结构、完整初稿、三路审稿、修改裁决、终稿修订、公众号成稿。artifact 的原始 JSON 默认折叠，但阶段说明始终可见，方便按问题类型调整来源、Prompt、模型、推理强度或人工反馈。
