...
9. W2 已增加统一 Pydantic 输出、一次结构修复、review issue 权限链和最终 claim-evidence matrix；C0 继续用于 W3 的真实成稿质量、风格与盲评对照，不能把结构通过等同于文章质量满分；
10. W3 已实现 12—20 标题候选、本地证据/套路/同质化过滤、读者第一眼 top 5、人工标题 preference、冻结授权短范例和模型稿到人工稿的服务端 diff；结构回归已经落地，但标题 ≥65% 与风格 ≥70% 仍只能由真实成对人工盲评确认，当前不得宣称达标。
